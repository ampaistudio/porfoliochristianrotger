...
import WallpapersSection from "./client/WallpapersSection";
import ContactModal from "./client/ContactModal";
  ...
  return (
    <div className="bg-[#09090b] min-h-screen text-stone-100 font-sans pb-4 transition-colors duration-200 selection:bg-stone-800 selection:text-white" ref={containerRef}>
      ...
      <header className="border-b bg-black/80 backdrop-blur sticky top-0 z-40" style={{ borderColor: brandColor }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {/* K7 — PWA install button (only shows when browser supports it) */}
            {canInstall && (
              <button
                onClick={promptInstall}
                className="hidden sm:flex items-center gap-1.5 text-[10px] font-mono font-bold text-stone-400 hover:text-white border border-stone-800 hover:border-stone-600 px-3 py-2 rounded-lg transition"
                title={lang === "es" ? "Instalar en pantalla de inicio" : "Add to home screen"}
              >
            ...
            {/* K1 — Contratar / Contact button */}
            <button
              onClick={() => setIsContactOpen(true)}
              className="text-white text-xs font-semibold px-3 sm:px-4 py-2 rounded-lg flex items-center gap-1.5 hover:opacity-90 transition shrink-0 border border-stone-700 hover:border-stone-500 bg-stone-900"
            >
              <Mail className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{lang === "es" ? "Contratar" : "Hire Me"}</span>
            </button>
            ...
            <button
              ...
              className="text-white text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-lg flex items-center gap-2 hover:opacity-95 transition shadow-lg shrink-0"
            >
              <Maximize2 className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{lang === "es" ? "Ver en Gran Slide" : "Enter Slideshow"}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 space-y-10">
          ...
          {filteredPhotos.map((photo, idx) => (
            ...
            >
              ...
              <div className="aspect-[4/3] bg-stone-950 relative overflow-hidden">
                ...
                {/* K8 — Dynamic watermark from config */}
                <span className="absolute bottom-3 right-3 z-10 text-[9px] sm:text-[10px] font-sans tracking-wide text-white/40 bg-black/35 px-1.5 py-0.5 rounded backdrop-blur-[1px] pointer-events-none select-none">
                  ©{config.photographerName} {new Date().getFullYear()}
                </span>
        ...
        {/* K10 — Wallpapers gratuitos */}
        <WallpapersSection
          wallpapers={publishedPhotos.filter(p => p.is_wallpaper)}
          lang={lang}
      ...
      {/* IMMERSIVE SLIDE PROJECTOR VIEW */}
      {isProjectorOpen && (
        <ClientSlideshow
