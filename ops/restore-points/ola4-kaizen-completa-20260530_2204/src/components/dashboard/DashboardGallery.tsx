...
import { savePhotoToSupabase, deletePhotoFromSupabase, deletePhotoFromStorage, uploadPhotoToStorage, checkDbConnection } from "../../utils/supabase";
...
export default function DashboardGallery({ photos, onUpdatePhotos, config, onUpdateConfig }: DashboardGalleryProps) {
  ...
  const handleMultipleFiles = async (fileList: FileList | File[]) => {
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      try {
        ...
        const photoId = `photo_${Date.now()}_batch_${i}_${Math.random().toString(36).substr(2, 5)}`;
        const cleanName = file.name.substring(0, file.name.lastIndexOf('.')) || file.name;
        let photoUrl: string, thumbUrl: string;
        try {
          const stored = await uploadPhotoToStorage(file, photoId);
          photoUrl = stored.url; thumbUrl = stored.thumbnailUrl;
        } catch {
          const [compressedUrl, tb] = await Promise.all([compressImage(file), generateThumbnail(file)]);
          photoUrl = compressedUrl; thumbUrl = tb;
        }
        const newPhoto: Photo = { id: photoId, url: photoUrl, thumbnail_url: thumbUrl, title: meta.title || cleanName, description: meta.description, category: newPhotoCategory, date: new Date().toISOString().split("T")[0], status: newPhotoStatus, camera: meta.camera || newPhotoCamera, lens: meta.lens || newPhotoLens, settings: meta.settings || newPhotoSettings, editorialReview: "", suggestedSettings: "" };
  ...
  const handleAddPhotoSubmit = async (e: React.FormEvent) => {
    ...
    let finalUrl = newPhotoUrl;
    let finalThumb = newPhotoThumbnailUrl || getThumbnailUrl(newPhotoUrl);
    const photoId = editingPhotoId || "photo_" + Date.now();

    if (newPhotoFile) {
      try {
        const stored = await uploadPhotoToStorage(newPhotoFile, photoId);
        finalUrl = stored.url; finalThumb = stored.thumbnailUrl;
      } catch {
    ...
    if (editingPhotoId) {
      const updatedPhoto: Photo = { id: editingPhotoId, url: finalUrl, thumbnail_url: finalThumb, title: newPhotoTitle, description: newPhotoDesc, category: newPhotoCategory, date: newPhotoDate, camera: newPhotoCamera, lens: newPhotoLens, settings: newPhotoSettings, editorialReview: newPhotoEditorial, suggestedSettings: newPhotoSuggested, status: newPhotoStatus, title_es: newPhotoTranslations.title_es, description_es: newPhotoTranslations.description_es, editorialReview_es: newPhotoTranslations.editorialReview_es, suggestedSettings_es: newPhotoTranslations.suggestedSettings_es, sortOrder: photos.find(p => p.id === editingPhotoId)?.sortOrder };
      ...
      const newPhoto: Photo = { id: photoId, url: finalUrl, thumbnail_url: finalThumb, title: newPhotoTitle, description: newPhotoDesc, category: newPhotoCategory, date: newPhotoDate || new Date().toISOString().split("T")[0], camera: newPhotoCamera, lens: newPhotoLens, settings: newPhotoSettings, editorialReview: newPhotoEditorial, suggestedSettings: newPhotoSuggested, status: newPhotoStatus, title_es: newPhotoTranslations.title_es, description_es: newPhotoTranslations.description_es, editorialReview_es: newPhotoTranslations.editorialReview_es, suggestedSettings_es: newPhotoTranslations.suggestedSettings_es, sortOrder: photos.length };
  ...
  const resetForm = () => {
    setNewPhotoUrl(""); setNewPhotoThumbnailUrl(""); setNewPhotoFile(null); setNewPhotoTitle(""); setNewPhotoDesc("");
  ...
  const handleDeletePhoto = async (id: string) => {
    await Promise.all([deletePhotoFromSupabase(id), deletePhotoFromStorage(id)]);
  ...
  const handleToggleWallpaper = async (photo: Photo) => {
    const updated = { ...photo, is_wallpaper: !photo.is_wallpaper };
    await savePhotoToSupabase(updated);
    onUpdatePhotos(photos.map(p => p.id === photo.id ? updated : p));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {photos.map((photo) => (
          <GalleryPhotoCard key={photo.id} photo={photo} config={config} onEdit={handleEditPhoto} onDelete={handleDeletePhoto} onDragStart={handleDragStart} onDrop={handleDropPhoto} onToggleWallpaper={handleToggleWallpaper} />
        ))}
        {photos.length === 0 && (
