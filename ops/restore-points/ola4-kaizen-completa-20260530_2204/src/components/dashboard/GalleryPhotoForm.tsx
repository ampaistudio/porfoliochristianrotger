...
export default function GalleryPhotoForm({
  ...
  const [aiStatus, setAiStatus] = useState<"idle" | "ok" | "error" | "no-url">("idle");
  ...
  return (
    <form onSubmit={handleAddPhotoSubmit} className="space-y-4">
      ...
      <div className="flex flex-col gap-2 border-t border-stone-100 pt-4 mt-2">
        <div className="flex items-center justify-between">
          <h3 className="text-xs uppercase tracking-wider font-mono font-medium text-amber-600 flex items-center gap-1.5">
            ...
            {geminiConfigured ? (
              <span className="ml-2 text-[9px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold uppercase">Gemini activo</span>
            ) : (
              <span className="ml-2 text-[9px] bg-stone-100 text-stone-500 px-2 py-0.5 rounded font-bold uppercase">Añadir VITE_GEMINI_API_KEY en .env</span>
            )}
          </h3>
          <button
            type="button"
            onClick={handleGenerateAI}
            disabled={isGeneratingAI || !newPhotoUrl || !geminiConfigured}
          ...
          >
            {isGeneratingAI ? "Analizando..." : "🪄 Auto-completar con IA (EN & ES)"}
          </button>
        </div>
        {aiStatus === "ok" && (
          <p className="text-xs text-emerald-600 font-mono">✅ Análisis curatorial generado en EN y ES con éxito.</p>
        )}
        {aiStatus === "no-url" && (
          <p className="text-xs text-amber-500 font-mono">⚠ Carga o vincula una foto primero para que la IA pueda analizarla.</p>
        )}
        {aiStatus === "error" && (
          <p className="text-xs text-red-500 font-mono">✗ Error IA: {aiError}</p>
        )}
      </div>
