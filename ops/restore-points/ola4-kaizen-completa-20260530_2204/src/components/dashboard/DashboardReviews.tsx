...
import { MessageSquare, Trash2, CheckCircle, XCircle, Zap, Image as ImageIcon, BarChart2, Clock } from "lucide-react";
...
import { useViewStatsPanel } from "../../hooks/usePhotoViewTracker";
...
export default function DashboardReviews({
  ...
  const viewStats = useViewStatsPanel();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* K9 — Photo view heatmap */}
      {viewStats.length > 0 && (
        <div className="border border-stone-200 rounded-xl p-5 space-y-3">
          <h3 className="font-sans font-semibold text-stone-800 flex items-center gap-2 text-sm">
            <BarChart2 className="w-4 h-4 text-stone-500" />
            Tiempo de Visualización por Foto
            <span className="text-[9px] font-mono text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded">Heatmap</span>
          </h3>
          <div className="space-y-2">
            {viewStats.slice(0, 8).map(stat => {
              const photo = photos.find(p => p.id === stat.photoId);
              const secs = Math.round(stat.totalViewMs / 1000);
              const maxMs = viewStats[0]?.totalViewMs || 1;
              const pct = Math.round((stat.totalViewMs / maxMs) * 100);
              return (
                <div key={stat.photoId} className="flex items-center gap-3">
                  {photo && (
                    <img src={photo.thumbnail_url || photo.url} alt="" className="w-8 h-8 rounded object-cover border border-stone-200 shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="text-xs text-stone-700 truncate">{photo?.title || stat.photoId.slice(0, 8)}</span>
                      <span className="text-[10px] font-mono text-stone-500 flex items-center gap-1 ml-2 shrink-0">
                        <Clock className="w-3 h-3" />{secs}s · {stat.viewCount}x
                      </span>
                    </div>
          ...
          <p className="text-[10px] text-stone-400 font-mono">
            Datos acumulados desde el navegador del visitante · Se actualizan al cerrar el slideshow.
          </p>
        </div>
      )}
