...
import { usePhotoViewTracker } from "../../hooks/usePhotoViewTracker";

const EMAILJS_SERVICE_ID = import.meta.env.VITE_EMAILJS_SERVICE_ID;
const EMAILJS_TEMPLATE_COMMENT = import.meta.env.VITE_EMAILJS_TEMPLATE_COMMENT;
const EMAILJS_PUBLIC_KEY = import.meta.env.VITE_EMAILJS_PUBLIC_KEY;

async function notifyNewComment(photoTitle: string, authorName: string, text: string): Promise<void> {
  if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_COMMENT || !EMAILJS_PUBLIC_KEY) return;
  try {
    await fetch("https://api.emailjs.com/api/v1.0/email/send", {
      ...
      body: JSON.stringify({
        ...
        template_id: EMAILJS_TEMPLATE_COMMENT,
        user_id: EMAILJS_PUBLIC_KEY,
        template_params: { photo_title: photoTitle, author_name: authorName, comment_text: text },
      }),
    });
  } catch {}
}
...
export default function ClientSlideshow({
  ...
  // K9 — track view time for current photo
  const activePhotoId = photos[currentSlideIndex]?.id ?? null;
  usePhotoViewTracker(isOpen ? activePhotoId : null);
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    // K2 — Email notification (silent, non-blocking)
    notifyNewComment(activePhoto.title || "", newCommentName, newCommentText);
        ...
        <div className={`transition-all duration-300 relative flex items-center justify-center overflow-hidden ${
          ...
          <div className="absolute bottom-4 right-4 pointer-events-none select-none z-10">
            <span className="text-[10px] sm:text-xs font-sans tracking-wide text-white/40 bg-black/45 px-2 py-1 rounded backdrop-blur-[1px] drop-shadow-md">
              ©{photographerName} {new Date().getFullYear()}
            </span>
          </div>
        </div>
