...
export function compressImageToBlob(file: File, maxWidth = 1000): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("Blob creation failed")), "image/jpeg", 0.80);
      };
...
export const generateThumbnailBlob = (file: File): Promise<Blob> => compressImageToBlob(file, 400);
