...
interface GalleryPhotoCardProps {
  ...
  onToggleWallpaper: (photo: Photo) => void;
}

export default function GalleryPhotoCard({
  photo, config, onEdit, onDelete, onDragStart, onDrop, onToggleWallpaper
    ...
    >
      <div className="aspect-[4/3] bg-stone-100 relative overflow-hidden group">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1">
          ...
          {photo.is_wallpaper && (
            <span className="bg-sky-600/90 text-white text-[9px] font-mono tracking-wider px-2 py-0.5 rounded uppercase font-bold shadow-sm backdrop-blur-sm w-max">
              🖼 Wallpaper
            </span>
          )}
        </div>
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
          ...
          <button
            onClick={() => onToggleWallpaper(photo)}
            className={`p-2.5 rounded-xl transition flex items-center gap-1.5 text-xs font-semibold cursor-pointer ${photo.is_wallpaper ? "bg-sky-600 text-white" : "bg-stone-700 text-stone-200"}`}
            title={photo.is_wallpaper ? "Quitar de Wallpapers" : "Marcar como Wallpaper"}
          >
